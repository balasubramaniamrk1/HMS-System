...
SSH_USER="${2:-root}"   # User to connect as for initial setup (default: root)
...
echo "Deploying to $SERVER_IP (Initial setup as $SSH_USER)..."

# 0. Bootstrap: User Creation & Dependency Installation
echo "==> Bootstrapping Server..."
ssh -t $SSH_USER@$SERVER_IP << EOF
    set -e
    
    echo "--> Installing System Dependencies..."
    # Update and Install Deps (Python, Postgres, Nginx, etc)
    # Check if apt is available and update
    if command -v apt &> /dev/null; then
        sudo apt update
        sudo apt install -y python3-pip python3-venv python3-dev libpq-dev postgresql postgresql-contrib nginx curl git acl
    else
        echo "Error: apt package manager not found. This script requires a Debian/Ubuntu based system."
        exit 1
    fi
    
    echo "--> Checking/Creating App User: $SERVER_USER..."
    if id "$SERVER_USER" &>/dev/null; then
        ...
        # Setup SSH access for the new user (Copy keys from current user)
        # Assuming we want to use the same key we used to login as SSH_USER
        sudo mkdir -p /home/$SERVER_USER/.ssh
        # If authorized_keys exists for SSH_USER, copy it.
        # This part assumes we connected with keys. 
        if [ -f ~/.ssh/authorized_keys ]; then
            ...
            echo "SSH keys copied to $SERVER_USER."
        fi
    fi
EOF
...
ssh -t $SERVER_USER@$SERVER_IP << EOF
    ...
    # Update and Install Deps (Moved to Bootstrap Phase)
    # Ensuring we are ready
    # echo "--> Installing System Dependencies..."
...
ENV
    ...
    echo "--> Configuring SSL Scripts..."
    cp $APP_DIR/production/setup_ssl.sh $PROJECT_DIR/setup_ssl.sh
    cp $APP_DIR/production/renew_ssl.sh $PROJECT_DIR/renew_ssl.sh
    chmod +x $PROJECT_DIR/setup_ssl.sh $PROJECT_DIR/renew_ssl.sh

    echo "==> Deployment Complete! Access at http://$SERVER_IP"
    echo "    To enable HTTPS, run: sudo $PROJECT_DIR/setup_ssl.sh <DOMAIN> [EMAIL]"
EOF
