...
1. **Test Gunicorn Manually** (Optional):
   ...
   ```
   *Visit `http://your_server_ip:8000` to verify it loads (you may need to allow port 8000 in firewall temporarily). Press Ctrl+C to stop.*

2. **Setup Systemd Service**:
   Copy the service file provided in the production folder to the systemd directory.
   
   *Check the file `production/gunicorn.service` first to ensure the paths match your installation (`/home/ubuntu/hms_project`).*
...
- **Static Files 404**:
  Ensure `STATIC_ROOT` in `settings.py` matches the Nginx config (`/home/ubuntu/hms_project/staticfiles`).
