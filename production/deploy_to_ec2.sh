...
ENV
    ...
    # Create Superuser if missing (Idempotent-ish check)
    echo "--> Checking Superuser..."
    $APP_DIR/venv/bin/python $APP_DIR/manage.py shell -c "from django.contrib.auth import get_user_model; User = get_user_model(); User.objects.filter(username='admin').exists() or User.objects.create_superuser('admin', 'admin@example.com', 'admin123')"

    # Gunicorn Setup
    echo "--> Configuring Gunicorn..."
    sed -i "s|User=ubuntu|User=$SERVER_USER|g" $PROJECT_DIR/production/gunicorn.service
    sed -i "s|/home/ubuntu/hms_project|$APP_DIR|g" $PROJECT_DIR/production/gunicorn.service
    sed -i "s|WorkingDirectory=.*|WorkingDirectory=$APP_DIR|g" $PROJECT_DIR/production/gunicorn.service
    sed -i "s|ExecStart=.*|ExecStart=$APP_DIR/venv/bin/gunicorn --access-logfile - --workers 3 --bind unix:/run/gunicorn/gunicorn.sock hms_project.wsgi:application|g" $PROJECT_DIR/production/gunicorn.service
    
    sudo cp $PROJECT_DIR/production/gunicorn.service /etc/systemd/system/gunicorn.service
    ...
    # sed -i "s|YOUR_DOMAIN_OR_IP|$DOMAIN_NAME|g" $PROJECT_DIR/production/nginx.conf
    sed -i "s|root /home/ubuntu/hms_project/staticfiles|root $APP_DIR/staticfiles|g" $PROJECT_DIR/production/nginx.conf
    sed -i "s|root /home/ubuntu/hms_project/media|root $APP_DIR/media|g" $PROJECT_DIR/production/nginx.conf
    
    sudo cp $PROJECT_DIR/production/nginx.conf /etc/nginx/sites-available/hms_project
...
# 3. SSL Setup (Interactive - run only if needed manually or non-interactive if automated)
# echo "==> [4/5] Setting up SSL..."
# ... (Leaving SSL setup as separate script or manual step to avoid re-running aggressive rate limits)
# If you want it automated, uncomment:
# $SSH_CMD -t $SERVER_USER@$SERVER_IP "sudo certbot --nginx -d $DOMAIN_NAME --non-interactive --agree-tos --email $EMAIL --redirect || echo 'Certbot failed or already present'"

echo "========================================================"
echo "Deployment Complete!"
echo "URL: http://$SERVER_IP (or https://$DOMAIN_NAME)"
echo "Log File: $LOG_FILE"
echo "========================================================"
